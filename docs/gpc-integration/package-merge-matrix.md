...
| `gpc_routes.py` | `veklom-byos-backend/backend/apps/gpc/routes.py` | defer/adapt | Routes must merge into the existing FastAPI router and remove mock auth/simulation. | Schemas, compiler, DB, auth | High |
| `gpc_routes_complete.py` | `veklom-byos-backend/backend/apps/gpc/routes.py` | defer/duplicate review | Complete route variant overlaps `gpc_routes.py`; not safe to install beside it. | Existing router registration | High |
| `gpc_schemas.py` | `veklom-byos-backend/backend/gpc/schemas.py` | defer/adapt | Schema source must align with existing BYOS `schemas.py`/`gpc_schemas.py`. | Pydantic, DB models | High |
